...
!function(app){"use strict";app.registerModule("shoppinglists")}(ApplicationConfiguration);
...
!function(){"use strict";function menuConfig(Menus){Menus.addMenuItem("topbar",{title:"Shopping Lists",state:"shoppinglists",type:"dropdown",roles:["*"]}),Menus.addSubMenuItem("topbar","shoppinglists",{title:"List Shopping Lists",state:"shoppinglists.list"}),Menus.addSubMenuItem("topbar","shoppinglists",{title:"Create Shopping List",state:"shoppinglists.create",roles:["user"]})}angular.module("shoppinglists").run(menuConfig),menuConfig.$inject=["Menus"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("shoppinglists",{"abstract":!0,url:"/shoppinglists",template:"<ui-view/>"}).state("shoppinglists.list",{url:"",templateUrl:"modules/shoppinglists/client/views/list-shoppinglists.client.view.html",controller:"ShoppinglistsListController",controllerAs:"vm",data:{pageTitle:"Shoppinglists List"}}).state("shoppinglists.create",{url:"/create",templateUrl:"modules/shoppinglists/client/views/form-shoppinglist.client.view.html",controller:"ShoppinglistsController",controllerAs:"vm",resolve:{shoppinglistResolve:newShoppinglist},data:{roles:["user","admin"],pageTitle:"Shoppinglists Create"}}).state("shoppinglists.edit",{url:"/:shoppinglistId/edit",templateUrl:"modules/shoppinglists/client/views/form-shoppinglist.client.view.html",controller:"ShoppinglistsController",controllerAs:"vm",resolve:{shoppinglistResolve:getShoppinglist},data:{roles:["user","admin"],pageTitle:"Edit Shoppinglist {{ shoppinglistResolve.name }}"}}).state("shoppinglists.view",{url:"/:shoppinglistId",templateUrl:"modules/shoppinglists/client/views/view-shoppinglist.client.view.html",controller:"ShoppinglistsController",controllerAs:"vm",resolve:{shoppinglistResolve:getShoppinglist},data:{pageTitle:"Shoppinglist {{ shoppinglistResolve.name }}"}})}function getShoppinglist($stateParams,ShoppinglistsService){return ShoppinglistsService.get({shoppinglistId:$stateParams.shoppinglistId}).$promise}function newShoppinglist(ShoppinglistsService){return new ShoppinglistsService}angular.module("shoppinglists").config(routeConfig),routeConfig.$inject=["$stateProvider"],getShoppinglist.$inject=["$stateParams","ShoppinglistsService"],newShoppinglist.$inject=["ShoppinglistsService"]}();
!function(){"use strict";function ShoppinglistsListController(ShoppinglistsService){var vm=this;vm.shoppinglists=ShoppinglistsService.query()}angular.module("shoppinglists").controller("ShoppinglistsListController",ShoppinglistsListController),ShoppinglistsListController.$inject=["ShoppinglistsService"]}();
!function(){"use strict";function ShoppinglistsController($scope,$state,$window,Authentication,shoppinglist){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.shoppinglist.$remove($state.go("shoppinglists.list"))}function addContent(isValid){function successCallback(res){$state.go("shoppinglists.view",{shoppinglistId:res._id})}function errorCallback(res){vm.error=res.data.message}return vm.contentList=vm.shoppinglist.contents,vm.contentList.push({content:vm.shoppinglist.content,priority:vm.shoppinglist.priority,notes:vm.shoppinglist.notes,isChecked:vm.shoppinglist.isChecked}),vm.shoppinglist.content="",vm.shoppinglist.priority="",vm.shoppinglist.notes="",vm.shoppinglist.isChecked=!1,isValid?void(vm.shoppinglist._id&&vm.shoppinglist.$update(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.shoppinglistcontentForm"),!1)}function deleteContent(content){function successCallback(res){$state.go("shoppinglists.view",{shoppinglistId:res._id})}function errorCallback(res){vm.error=res.data.message}vm.contentList=vm.shoppinglist.contents;var contenttodelete=vm.contentList.indexOf(content);vm.shoppinglist.contents.splice(contenttodelete,1),vm.shoppinglist.contents=vm.contentList,vm.shoppinglist._id&&vm.shoppinglist.$update(successCallback,errorCallback)}function deleteSelectedContent(){function successCallback(res){$state.go("shoppinglists.view",{shoppinglistId:res._id})}function errorCallback(res){vm.error=res.data.message}vm.contentList=vm.shoppinglist.contents;for(var i=vm.shoppinglist.contents.length-1;i>-1;i--)vm.shoppinglist.contents[i].isChecked&&(vm.shoppinglist.contents.splice(i,1),vm.shoppinglist.contents=vm.contentList);vm.shoppinglist._id&&vm.shoppinglist.$update(successCallback,errorCallback)}function save(isValid){function successCallback(res){$state.go("shoppinglists.view",{shoppinglistId:res._id})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.shoppinglist._id?vm.shoppinglist.$update(successCallback,errorCallback):vm.shoppinglist.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.shoppinglistForm"),!1)}var vm=this;vm.authentication=Authentication,vm.shoppinglist=shoppinglist,vm.error=null,vm.form={},vm.remove=remove,vm.save=save,vm.contentList=[],vm.addContent=addContent,vm.deleteSelectedContent=deleteSelectedContent,vm.deleteContent=deleteContent}angular.module("shoppinglists").controller("ShoppinglistsController",ShoppinglistsController),ShoppinglistsController.$inject=["$scope","$state","$window","Authentication","shoppinglistResolve"]}();
!function(){"use strict";function ShoppinglistsService($resource){return $resource("api/shoppinglists/:shoppinglistId",{shoppinglistId:"@_id"},{update:{method:"PUT"}})}angular.module("shoppinglists").factory("ShoppinglistsService",ShoppinglistsService),ShoppinglistsService.$inject=["$resource"]}();
